...
export default class AtomWindowsTitlebarTitleBar {
  ...
  addEvents() {
    ...
    let unmaximizedBounds;
    let maximizedBounds;
    let savedBounds = atom.config.get('atom-windows-titlebar-edit.unmaximized_bounds');
    if (savedBounds) {
      unmaximizedBounds = JSON.parse(savedBounds);
    }
    let setBounds = false;
    let updateStyling = () => {
      let maximized = browserWindow.isMaximized();
      if (maximized) {
        document.body.classList.remove('unmaximized');
        document.body.classList.add('maximized');
      } else {
        document.body.classList.add('unmaximized');
        document.body.classList.remove('maximized');
      }
    };
    let unmaximize = function() {
      //console.log(maximizedBounds);
      browserWindow.unmaximize();
      if (!setBounds && unmaximizedBounds) {
        setBounds = true;
        console.log('SETTING WINDOW BOUNDS');
        browserWindow.setBounds(unmaximizedBounds);
      }
      updateStyling();
    }
    ...
    this.browserWindow.addListener('resize', () => {
      if (browserWindow.isMaximized()) {
        maximizedBounds = browserWindow.getBounds();
      } else {
        unmaximizedBounds = browserWindow.getBounds();
        atom.config.set('atom-windows-titlebar-edit.unmaximized_bounds', JSON.stringify(unmaximizedBounds))
      }
    });
    this.controls = {
  ...
  buildSubMenuHTML(submenu) {
    var html = ''
    for (var item of this.filterMenus(submenu)) {
